...
- `OPENCLAW_GATEWAY_TOKEN` (required for Control UI — generate with `openssl rand -hex 32`; treat as an admin secret)

### Trusted Proxies

Railway routes traffic through internal reverse proxies (`100.64.0.0/10`).
To allow the gateway to correctly identify client IPs behind Railway's proxy,
add the following to your `openclaw.json` config (created by the setup wizard under `/data/.openclaw/`):
...
- `trustedProxies` — without this, the gateway logs `Proxy headers detected from untrusted address` warnings.
- `dangerouslyDisableDeviceAuth` — disables device pairing for the Control UI. On Railway there is no local/CLI access to approve pairing requests, so gateway token auth is used instead.

> **Note:** Fresh deployments seed this config automatically via the Docker entrypoint. If you already have an `openclaw.json`, merge the keys above into it.
